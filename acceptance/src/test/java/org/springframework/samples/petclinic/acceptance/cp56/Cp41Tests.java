...
/** cp41 code-collision: UPDATED by cp56 (member-id) — Unify the customerCode and membershipNumber into a single 'memberId' formatted '<REGION><FY><HASH8><CHK>' (region code, 2-digit fiscal year, 8 hex hash, 1 Luhn check digit), and remove the separate customerCode and membershipNumber fields */
@Tag("cp41")
class Cp41Tests extends AcceptanceBase {
	...
	void coreUpdatedBehaviour() throws Exception {
		...
		// TODO: assert the UPDATED behaviour of "code-collision" under the new spec.
