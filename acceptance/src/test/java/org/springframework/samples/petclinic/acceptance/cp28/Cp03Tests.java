...
/** cp03 telephone-unique: UPDATED by cp28 (identity-key) — Consolidate all duplicate detection into a single derived 'identityKey' = normalizedTelephone + '|' + (email or empty) + '|' + householdId, and reject with 409 when a new owner's identityKey matches an existing owner */
@Tag("cp03")
class Cp03Tests extends AcceptanceBase {
	...
	void coreUpdatedBehaviour() throws Exception {
		...
		// TODO: assert the UPDATED behaviour of "telephone-unique" under the new spec.
